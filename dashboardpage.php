...
.header-title {
    ...
    margin: 20px 0; /* Adjust margin as needed */
                ...
                .card-container a {
                    text-decoration: none; /* Remove underline from anchor tags within card-container */
                }
...
<div class="dash-left-container">
   ...
   <?php
            ...
            $totalNotesQuery = "SELECT COUNT(*) AS total_notes_count FROM tbl_notes WHERE id = $userId AND archived = 0";
            ...
            $totalFavoritesQuery = "SELECT COUNT(*) AS total_favorites_count FROM tbl_notes WHERE id = $userId AND favorite = 1 AND archived = 0";
...
<div class="card-container">
    <a href="notebook.php" class="card">
        <h2>Total Notes</h2>
        <p><?php echo $totalNotesCount; ?></p>
    </a>
    <a href="favorite.php" class="card">
        <h2>Total Favorite</h2>
        <p><?php echo $totalFavoritesCount; ?></p>
    </a>
    <a href="archivepage.php" class="card">
    ...
    // Function to add hover animation to a card
    function addHoverAnimation(event) {
        event.target.classList.add('hover-animation');
    }

    // Function to remove hover animation from a card
    function removeHoverAnimation(event) {
        event.target.classList.remove('hover-animation');
    }


    // Add event listeners for hover animation and redirection to each card
    cards.forEach(function(card) {
        card.addEventListener('mouseenter', addHoverAnimation);
        card.addEventListener('mouseleave', removeHoverAnimation);
    });
});
